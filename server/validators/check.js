...
const validate = (data, field_name, rules) => {
    return new Promise((resolve, reject) => {
        ...
        for(let i = 0; i < ExtractedRules.length; i++) {
            console.log(RulesFunctions[ExtractedRules[i].split(':')[0]](data[field_name], ExtractedRules[i].split(':')[1]))


            RulesFunctions[ExtractedRules[i].split(':')[0]](data[field_name], ExtractedRules[i].split(':')[1])
            .then(result => {
